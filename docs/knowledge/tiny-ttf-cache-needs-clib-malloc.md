...
The shared font code in the screens submodule (`components/seedsigner/font_registry.cpp`, `gui_constants.cpp`) creates
...
`LV_ASSERT_HANDLER` = `while(1);` busy-loops → **CPU-bound hang.** Root cause (with repro) in the screens
submodule:
[`sources/seedsigner-lvgl-screens/docs/knowledge/tiny-ttf-cache-spin-root-cause.md`](../../sources/seedsigner-lvgl-screens/docs/knowledge/tiny-ttf-cache-spin-root-cause.md).
...
`LV_MEM_SIZE` raised to a few MB — CLIB is simpler and matches the screens repo's desktop tools.)
...
This must move **in lockstep** with the screens submodule pin: any commit that bumps it carries
the cache-on-by-default font code, so this override must already be present (it is). If a future
memory-tight variant ever needs the cache off, build the screens code with `-DSEEDSIGNER_TTF_CACHE_SIZE=0` rather
